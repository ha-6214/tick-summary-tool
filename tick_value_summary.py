...
# -*- coding: utf-8 -*-
"""
歩み値CSV 固定集計スクリプト v3.0（基本情報の追記対応）
──────────────────────────────────
...
使い方（コードは一切変更せず、引数で値を渡す）:
  ...
  任意のオプション引数（--キー=値 の形式。順不同・省略可）:
    ...
    --code=6758                    銘柄コード
    --name=ソニーグループ           銘柄名称
    --yearhigh_date=2026年1月24日  年初来高値の日付
    --margin_sell=238000           信用売れ残
    --margin_buy=9787200           信用買い残
    --margin_ratio=41.12           信用倍率
    --threshold=5000000            機関/個人の閾値（円・省略時500万円）

  例:
    python3 tick_value_summary.py /path/qr-6758-20260717.csv 4124 \\
      --date=2026年7月17日 --code=6758 --name=ソニーグループ \\
      --yearhigh_date=2026年1月24日 --margin_sell=238000 \\
      --margin_buy=9787200 --margin_ratio=41.12
"""
